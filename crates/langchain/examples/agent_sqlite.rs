...
use langgraph::checkpoint::checkpoint_struct_api::{SqliteSaver, SqliteSaverConfig};
...
async fn main() {
    ...
    let checkpointer = Arc::new(
        SqliteSaver::new(SqliteSaverConfig::default())
            .await
            .unwrap(),
    );
    ...
    let result = agent
        .invoke(Message::user("我给你取名叫老大！"), Some("sqlite001"))
        .await
        .unwrap();
